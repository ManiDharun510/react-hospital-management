...
function Doctordetails(){
    ...
    const editingBindHandler = (index)=>{
        ...
        seteditInput({
            ...
            doctoremail: doctorList[index].doctoremail,
            speciality: doctorList[index].speciality
        })
    };

    const onEditSubmitHandler = ()=>{
        axios.put(`http://localhost:4000/updatedoctor/${doctorList[getId]._id}`,{...editInput}).then(()=>{
            getdoctorlistapi();
        }).catch(()=>{
    ...
    const onDeleteHandler = (index)=>{
        axios.delete(`http://localhost:4000/deletedoctor/${doctorList[index]._id}`).then(()=>{
            getdoctorlistapi();
            navigate('/dd');
        }).catch(()=>{
    ...
    {doctorList.map((val, index)=>{
        ...
        <td><button className="btn btn-primary" onClick={()=>{editingBindHandler(index)}} data-toggle="modal" data-target="#doctor">Edit</button></td>
    ...
    {/* ----------------------------------------------------------Editing Modal------------------------------ */}
    <div className="modal bd-example-modal-md" id="doctor" tabIndex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
